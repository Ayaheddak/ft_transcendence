...
export class GameService {
    ...
    */
    ...
    async UserIngame(id: number, inf: GameObj, io : Server) {
        try {
            let LeftUser: User = await this.UserRepo.findOneBy({ id: id})
            if (LeftUser === undefined)
                throw ("waywa ga3ma kayn")
            LeftUser.status = "in game"
            await this.UserRepo.save(LeftUser)
        } catch (e) {
            this.salat(inf, 1, io) //TODO protect
            io.in(inf.state.roomid).emit("salat", )
        }
    }
    ...
    async Ball_Logic(io: Server, inf: GameObj) {
        ...
        */
        await this.UserIngame(inf.left_plr.Player.id, inf, io)
        await this.UserIngame(inf.right_plr.Player.id, inf, io)
