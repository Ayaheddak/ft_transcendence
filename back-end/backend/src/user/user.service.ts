...
export class UserService {
  ...
  async createUser(name: string, email: string, avatar: string, fullname: string): Promise<User> {
    const user = new User();
  ...
  async findUserByUn(username: string): Promise<User | undefined> {
    ...
    return  await this.userRepository.findOne({where : {username: username}, relations: ['friendship_sender', 'friendship_reciver', 'receivedMessages', 'receivedMessages.SenderUserID', 'messageid', 'blockedList', 'blockedList.BlockedUser', 'blocker', 'blocker.BlockedUser', 'Notifreciever', 'Notifreciever.userid1', 'gameStats']});
  }
 ...
 const users = await this.userRepository.find();
  ...
  async findAllUsers(): Promise<User[]> {
    const user = await this.userRepository.find({relations: ['friendship_sender.userid1', 'friendship_sender.userid2', 'friendship_reciver.userid1', 'friendship_reciver.userid2', 'blockedList', 'blockedList.BlockedUser', 'blocker', 'blocker.BlockedUser', 'gameStats']}); 
    return user; 
  }
