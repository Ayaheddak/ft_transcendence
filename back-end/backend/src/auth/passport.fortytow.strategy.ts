...
export class FortyTwoStrategy extends PassportStrategy(Strategy, 'FortyTwo') {
  ...
  async validate(accessToken: string, refreshToken: string, profile: any) : Promise<any> {
    ...
    const {user , isFirst}  = await this.AuthService.login({email: profile.emails[0].value, username: profile.emails[0].value.split("@")[0] + "_42", avatar: profile._json.image.link, fullname: profile.displayName});
    // if (!user) return null;
    // console.log(user);
    // console.log("profile here");
